...
import { CarRecord } from "@/lib/api-client";

interface EditCarModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdateCar: (carData: CarRecord) => void;
  car: CarRecord | null;
}
...
export function EditCarModal({ isOpen, onClose, onUpdateCar, car }: EditCarModalProps) {
  ...
  const handleChange = (field: keyof CarRecord, value: string | number) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.mileage === undefined || formData.mileage < 0) {
      newErrors.mileage = "주행거리는 0 이상이어야 합니다";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await onUpdateCar(formData as CarRecord);
      onClose();
    } catch (error) {
        ...
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <CardTitle className="text-xl">차량 정보 수정</CardTitle>
                <CardDescription>
