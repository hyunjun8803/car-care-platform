...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('=== Supabase expenses 테이블 생성 시작 ===');
    ...
    try {
      ...
      if (tableError) {
        ...
        console.log('expenses 테이블 생성 성공');
    ...
    return NextResponse.json({
      ...
      message: allSuccessful ? 'expenses 테이블이 성공적으로 생성되었습니다!' : 'expenses 테이블 생성 중 일부 오류가 발생했습니다.',
      results,
      tableExists,
      nextStep: allSuccessful ? '이제 차계부 데이터가 Supabase에 영구 저장됩니다.' : 'Supabase 대시보드에서 수동으로 create-expenses-table.sql을 실행하세요.',
      manualSQL: allSuccessful ? null : 'create-expenses-table.sql 파일을 확인하세요.',
      timestamp: new Date().toISOString()
    });

  } catch (error) {
    console.error('expenses 테이블 생성 실패:', error);
    return NextResponse.json({
      success: false,
      error: 'expenses 테이블 생성 실패',
      details: error instanceof Error ? error.message : String(error),
      suggestion: 'Supabase 대시보드에서 create-expenses-table.sql 파일의 SQL을 수동으로 실행하세요.'
    }, { status: 500 });
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      tableExists,
      tableError,
      message: tableExists ? 'expenses 테이블이 존재합니다.' : 'expenses 테이블이 존재하지 않습니다.',
      action: tableExists ? '정상적으로 차계부 데이터를 저장할 수 있습니다.' : 'POST 요청으로 테이블을 생성하거나 Supabase 대시보드에서 수동 생성하세요.',
      timestamp: new Date().toISOString()
    });
